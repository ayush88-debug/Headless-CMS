...
function renderPricingPlans(plans) {
 ...
 function renderTestimonials(testimonials) {
  ...
  function renderFaqs(faqs) {
    const container = document.querySelector('#faqs-container'); // Add this ID
    ...
    faqs.forEach((faq, index) => {
        ...
        // Simple display for now:
         const faqHTML = `
         <div class="w-full px-4 ${index >= faqs.length / 2 ? 'lg:w-1/2' : 'lg:w-1/2'}"> {/* Basic column split */}
